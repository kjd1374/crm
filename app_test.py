...
from database_test import get_db, init_db
from models_test import Customer, Order, Interaction, Quote
import utils_test as utils

# Page Config
st.set_page_config(page_title="간편 CRM (Test Mode)", layout="wide", page_icon="🧪")
...
# --- Sidebar Navigation ---
st.sidebar.title("🧪 CRM (테스트)")
st.sidebar.caption("견적 기능 테스트 버전입니다.")
        ...
        with col_main:
            ...
            with sub_tab2: # Orders
                ...
                if orders:
                    ...
                    st.dataframe(df_orders, column_config={
                        "날짜": "날짜",
...
# --- PAGE 3: Quote Management (NEW) ---
                                ...
                                for idx, opt_group in enumerate(options_list):
                                    ...
                                    with opt_cols_gen[idx]:
                                        ...
                                        if th > 0 and i_qty <= th:
                                            ...
                                            # And do NOT add unit price? Or add 0 unit price?
                                            # Strategy: Usually unit price is still base, but options fee is lumpsum.
                                            # User said: "If 500 or less, printing fee is 250,000 fixed added"
                                            # So we assume option price per unit is 0 in this case.
                                        else:
    ...
    with tab_list:
        ...
        # We need a proper delete function here
        customers = utils.get_all_customers(db)
        if customers:
            for c in customers:
    ...
    with tab_prod:
        ...
        with col_form:
            with st.container(border=True):
                ...
                if sourcing_type == "국내":
                    # Domestic: Simple Price, No Description Input
                ...
                with st.expander("옵션 그룹 추가/관리", expanded=True):
                    with st.form("add_opt_form", clear_on_submit=True):
                        ...
                        # Default threshold changed to 499 for Domestic as requested
                        # (Applying to China too unless specified, but user said 'for Domestic... change to 499'. 
                        #  I will stick to 499 as a smart default for both or just use 499.)
...
# --- PAGE 4: Data Entry ---
    ...
    st.title("📝 데이터 입력")
    # Using existing logic but imported from utils_test
        ...
        with st.form("manual_customer_form"):
            ...
            if submitted:
                if not m_company:
                    ...
                    db = get_session()
                    try:
                        customer_data = {"company_name": m_company, "client_name": m_client_name, "phone": m_phone, "industry": m_industry, "sales_rep": m_sales_rep}
                        ...
                        if existing:
                            ...
                            if m_product or m_total > 0:
                                utils.create_order(db, new_customer.id, date.today(), m_product, m_qty, m_total, 0, "첫 수동 등록")
